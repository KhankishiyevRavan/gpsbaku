...
export default function UpdateContractForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // if(contractId)
      if (!contractId) return;
      const data = await updateContract(contractId, formData);
  ...
  const handleSelectChangePackageDate = (value: string) => {
    const durationInYears = parseInt(value); // 1, 2, 3
    const startDate = new Date(formData.startDate);
    // Əgər başlanğıc tarixi yoxdursa, endDate təyin etmə
    ...
    setFormData((prevData) => ({
      ...prevData,
      contractDuration: value,
      endDate: calculatedEndDate.toISOString().split("T")[0], // YYYY-MM-DD
    }));
  };
  const setAddress = () => {};
          ...
          <DatePicker
            ...
            onChange={(dates, currentDateString) => {
              ...
              if (duration && currentDateString) {
                const start = new Date(currentDateString);
                endDate = addYears(start, duration).toISOString().split("T")[0];
              }
