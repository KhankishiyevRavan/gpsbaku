...
import { getAllUsers, userDataInterface } from "../services/userService";
import {
  Table,
  TableBody,
  TableCell,
...
interface ChooseContractUserProps {
  navigateBasePath: string;
}

const ChooseContractUser = ({ navigateBasePath }: ChooseContractUserProps) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // console.log(data.filter((d) => d.role == "abuneci"));
      let newData = data.filter((d) => d.role == "abuneci");
      setUsers(newData);
    } catch (err: any) {
        ...
        <Table>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {currentUsers.map((user, index) => (
              <TableRow
                key={index}
                onClick={() => navigate(`${navigateBasePath}/${user._id}`)}
...
export default ChooseContractUser;
