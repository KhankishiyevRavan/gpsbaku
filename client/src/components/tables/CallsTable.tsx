...
// import { roleDataInterface } from "../../services/roleService";
// import { useNavigate } from "react-router";
import { CallResponse, getAllCalls } from "../../services/callService";

export default function CallsTable() {
  // const navigate = useNavigate();

  const [operators, setOperators] = useState<CallResponse[]>([]);
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {operators?.map((operator) => (
              <TableRow key={operator?._id}>
                ...
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  {operator.assignedTechnician?.fname
                    ? `${operator.assignedTechnician?.fname} ${operator.assignedTechnician.lname} `
                    : "Yoxdur"}
                </TableCell>
                {/* <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  {operator.fields.length}
                </TableCell> */}
                {/* <TableCell>
