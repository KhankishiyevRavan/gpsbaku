...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      // console.log("Permissions from role:", permissions);
  ...
  try {
    ...
    res.status(200).json({
      ...
      user: {
        ...
        accountName:user.accountName ||"yoxdur",
        fname: user.fname,
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    // Əgər admin-disə, bütün istifadəçiləri (özündən başqa) çək
    if (requesterRole.name === "admin") {
      const users = await User.find({ _id: { $ne: req.user.userId } }).populate(
        "role"
      );
    ...
    // Admin deyilsə – read icazəsi olan rolları tap
    const permissionsRoles = requesterRole.permissions.roles || [];

    // read icazəsi olan rolların ObjectId-lərini topla
...
exports.editUser = async (req, res) => {
  ...
  try {
    ...
    user.measurementSystem = measurementSystem|| user.measurementSystem;
